...
/// Serializes model weights to a file using `bincode` (binary, compact, fast).
#[allow(dead_code)]
pub fn save_checkpoint<T: serde::Serialize>(data: &T, path: &Path) -> anyhow::Result<()> {
    ...
    tracing::info!(path = %path.display(), size_bytes = bytes.len(), "Checkpoint saved");
    Ok(())
}

/// Deserializes model weights from a binary checkpoint file.
#[allow(dead_code)]
